...
스튜디오의 **1단계(설정) → 프로젝트 열기**는 파일 묶음 하나로 export 전 단계까지 완성된 프로젝트(슬라이드 + 로케일별 스크린샷 + 캡션/번역)를 조립한다. AI 에이전트는 아래 형식으로 폴더에 파일을 준비하고, 사람은 그 파일들을 **한 번에 다중 선택**하면 된다 (ZIP 아님). 에디터가 검수 단계다.

> MCP 클라이언트는 이 문서를 `get_import_spec` 도구로 그대로 받고, 준비한 폴더를 `validate_import` → `render`로 돌린다 — MCP 도구 전체 목록은 [docs/mcp-server.md](./mcp-server.md).
...
| `themeBackground` | string \| object | 레퍼런스 그라디언트 | 문자열 = 테마 프리셋 id. 객체 = `{"type":"solid","color":"#…"}` 또는 `{"type":"gradient","gradient":{"direction":145,"stops":[{"color":"#…","position":0},…]}}`. `image` 불가. 옵션 오버레이(베이스 위에 겹침): `blobs` — 부드러운 방사형 컬러 스팟(mesh 그라디언트 룩) 배열 `[{"color":"#…","x":0.2,"y":0.15,"radius":0.5,"opacity":0.55,"blendMode":"overlay"}]`, 최대 6개, `x`/`y`/`radius`는 캔버스 비율(범위 밖 보정), `blendMode`는 `multiply`·`screen`·`overlay`·`soft-light`(생략 = 표준 알파 블렌드) — 와 `noise` — 필름 그레인 강도 0~1 |
...
UI(예: A/B 컨트롤, 녹음 버튼)를 키워 강조할 때 강력하다. 슬라이드당 최대 2개.
  ...
  "highlights": [
    {
      ...
      "marker": { "show": true, "color": "#6366F1" },  // 원본 테두리(생략 시 기본 점선, null이면 없음)
      "popup": {
        "x": 0.62,        // 카드 중심 X(캔버스 비율 0–1). 생략하면 sourceRegion 중심
        "y": 0.34,        // 카드 중심 Y(캔버스 비율 0–1)
        "zoom": 2.5,       // 배율(1–6). 카드 크기는 여기서 나온다 — 권장 노브
        "rim": { "color": "#FFFFFF", "width": 0.006 },  // 카드 테두리(생략 시 흰 테두리, null이면 없음)
        "auto": false,      // 자동 배치(x/y를 주면 기본 off, 안 주면 기본 on)
        "connector": true,  // 원본 → 카드 연결선(기본 on)
        "shape": "circle",  // 카드 실루엣: rect(기본) | circle(짧은 변의 절반으로 둥글림)
...
- **카드 크기는 `popup.zoom`으로 잡는다.** 배율은 영역이 슬라이드에 *그려진* 크기의
  배수라서, `sourceRegion`을 다시 잡아도 배율이 유지된다. `popup.width`(캔버스 너비
  비율, 0.1–1.5)는 zoom 이전의 크기 지정 방식으로 남아 있다 — **`width`만 주면 그 값이
  그대로 쓰이고 zoom·rim 기본값은 채워지지 않는다**(기존 매니페스트가 움직이지 않도록).
  둘 다 주면 zoom이 이긴다.
- `popup.rim`은 카드 테두리. 같은 색 UI 위에 뜬 카드는 테두리가 없으면 경계가 안 보인다.
  `width`는 캔버스 너비 비율(도형 strokeWidth와 같은 단위). `null`이면 테두리 없음.
- **`popup.x/y`를 생략하면 `auto: true`가 붙어 레이아웃이 카드를 놓는다** — 원본 영역을
  가리지 않고 캡션을 피하는 쪽(위/아래 중 충돌이 적은 쪽), 가로는 캔버스 중앙, 안전
  여백 안쪽. `x/y`를 주면 그 자리에 고정된다(에디터에서 카드를 끌어도 고정으로 바뀐다).
  `auto`가 켜져 있는 동안 `x/y`는 렌더된 위치의 기록일 뿐 입력이 아니다.
- `popup.connector`는 원본 마커에서 카드로 가는 연결선. 마커와 같은 잉크(같은 색·점선·
  두께)로 그려지고, 카드가 원본을 덮거나 `marker.show`가 false면 그리지 않는다 —
  안 그려진 테두리에서 뻗어나온 선은 단서가 아니라 잡티로 읽힌다.
- `marker`는 원본 영역에 그려지는 테두리이자 연결선의 잉크. `show: false`(또는 `null`)면
  카드만 남는다 — 이때 원본 박스가 캔버스에 없으므로 `highlight-popup-source-overlap`
  검사도 함께 사라진다(카드를 원본 위에 겹치는 in-place 렌즈 연출이 가능해진다).
- `popup.shape: "circle"`은 모서리를 짧은 변의 절반으로 둥글린다 — 정사각 카드는 원,
  가로로 긴 카드는 알약 모양. 원을 원하면 `sourceRegion`을 정사각에 가깝게 잡을 것.
- 나머지 필드도 모두 옵션 — 빠지면 makeHighlight 기본값으로 채워진다. 범위 밖 값은
  경계값 보정.
...
사람의 손: 스튜디오 1단계에서 "프로젝트 열기 → 파일 선택"으로 위 파일 전부 선택 → 요약 모달 확인 → "에디터에서 검수".
...
- `--bundle`은 렌더를 건너뛰고 편집 가능한 프로젝트 번들(`<out-dir>/<name>.studio.zip`)을
  저장한다. PNG가 아니라 에디터에서 "프로젝트 열기"로 다시 열어 손볼 산출물이
