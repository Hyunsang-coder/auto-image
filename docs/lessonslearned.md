...
### 미리보기는 에디터 해상도로 렌더 — 단, export 미리보기는 예외
...
- **4단계(내보내기)의 미리보기는 일부러 폭을 안 넘긴다.** fit-to-box 하한이나 헤드라인 gap 같은 절대 픽셀 상수가 스케일마다 다르게 걸려서, 축소 렌더는 실제 PNG와 어긋난다. 출고 직전에 눈으로 확인하는 화면이라 속도보다 일치가 우선이다 — "느리니까 previewWidth 주자"는 최적화가 바로 이걸 깨뜨린다.
- 두 경우 모두 `useSlideThumbnails(slides, locale, width?)` 하나를 쓴다(디바운스 + 콘텐츠 해시 캐시 + 슬라이드별 커밋). 렌더러를 새로 만들지 말 것.
