...
> **상태: Phase 0 · 1a · 1b · 1c · 2 전부 구현됨** (2026-09-01, 커밋 `89cc4a2`,
> `b56dae1`). 아래 완료 기준의 체크박스는 그 검증 결과이고, 자동화할 수 없어 사람
> 손이 필요한 항목만 맨 끝 「사람이 확인할 것」에 남겼다. 구현된 코드의 지도는
> `CLAUDE.md`의 *Document model (desktop)* 절, 결정의 근거는 `docs/adr.md`
> ADR-009…013.
...
## 먼저: 스파이크 (반나절, 코드 남기지 않아도 됨) — 결과

Phase 1의 위험이 여기 몰려 있다. **이걸 먼저 확인하지 않고 1b를 설계하지 말 것.**

> **스파이크 결론.**
> 1. **종료 가드 왕복은 동작한다.** 다만 `PredefinedMenuItem::quit`으로는 안 된다 —
>    macOS native quit(과 `tell application … to quit` AppleEvent)은 창 이벤트를
>    아예 거치지 않아 가드를 통째로 건너뛴다. 그래서 ⌘Q는 **커스텀 메뉴 항목**으로
>    만들어 `quit::request`를 태운다. 3초는 결정 시한이 아니라 **생존 확인**으로
>    두었다(ADR-010): 웹뷰가 `close_ack`로 살아 있음만 증명하면 그 뒤로는 시한이
>    없어, 사용자가 프롬프트를 읽는 시간도 10초짜리 저장도 잘리지 않는다. 재진입은
>    `asking` 플래그로, 취소 후 재요청은 `generation` 카운터로 막는다.
> 2. **`.studio.zip` 이중 확장자는 코드에서 정규화한다.** macOS 저장 패널이 무엇을
>    돌려주든 `ensureDocExt`가 `.studio.zip`을 되붙이고, 그 결과가 패널이 확인하지
>    않은 다른 파일을 가리키면 **앱이 직접 덮어쓰기를 묻는다.** 확장자를 바꾸지
>    않았으므로 `scripts/`·`docs/`·MCP는 손대지 않았다.
...
- [x] 새 프로젝트를 만들면 기본 폴더에 `.studio.zip`이 생기고 헤더가 그 파일명을 보여준다 — `test:doc`
- [x] 편집 → ⌘S → 앱 종료 → 재실행 → 같은 문서가 같은 내용으로 열린다 — `test:doc`(저장 → 다른 문서 → 되돌아오기), `docPath`/`savedHash`가 `partialize`에 포함
- [x] 편집 후 undo로 저장 시점까지 되돌리면 dirty가 **스스로 풀린다** — `documentModel.test.ts`(해시 비교 + `updatedAt` 제외). 캔버스 undo 자체의 왕복은 기존 e2e가 덮는다
- [x] 저장 실패(권한 없는 경로)가 모달로 뜨고, **원본 파일이 그대로다** — `test:doc`(읽기 전용 폴더), `documentIO.test.ts`, `document.rs`
- [x] v4 번들을 열면 clean이고, 첫 저장에서만 `.bak`이 생긴다 — `test:doc`(바이트 비교까지), `document.rs`
- [x] 두 번째 인스턴스가 뜨지 않는다 — 실기 확인(창을 만들기 전에 종료), `instance.rs`
...
- [x] **A → B 전환이 ⌘O + 클릭 1번** (현재 7번) — ⌘O 피커의 Recents 행 = 클릭 1번
- [x] 스텝 2·3·4 어디서든 ⌘O가 열린다 — 네이티브 메뉴 액셀러레이터라 스텝과 무관
- [x] dirty 상태에서 전환하면 3버튼이 뜨고, `저장`이 실제로 저장한 뒤 전환한다 — `documentIO.test.ts`, `test:doc`(미저장 상태의 `open` 거부)
- [x] 파일을 Finder에서 지운 뒤 Recents를 열어도 앱이 죽지 않는다 — `loadRecents`가 항목마다 `document_exists`로 확인해 회색 + 「찾을 수 없음」 + 「목록에서 제거」; `markMissing`/`parseRecents` 유닛
- [x] 목록을 그릴 때 `.studio.zip`을 하나도 열지 않는다 (preview 캐시가 도는지 확인) — `test:doc`이 recents.json의 모든 항목에 `preview`가 있는지 확인한다
...
- [x] 저장 10번 뒤 백업이 10개이고 11번째에 가장 오래된 것이 밀려난다 — `document.rs` 유닛(`rotation_keeps_the_newest_and_pushes_the_oldest_out`)
- [x] 백업에서 되살린 프로젝트에 **이미지가 들어 있다** — 백업은 `exportProjectBundle`이 만든 같은 번들이고, 왕복은 `projectBundle.test.ts`가 이미지 표면 전부(테마 배경 · 로케일 오버라이드 · 외부 이미지 포함)로 검증한다
- [x] 라이브러리 항목이 이미지까지 포함해 파일로 나가고 Recents에 나타난다 — 실기 확인(`Bridge Test.studio.zip`이 첫 실행에 생성되고 Recents에 들어감)
- [x] 마이그레이션을 두 번 돌려도 중복이 생기지 않는다 — 완료 플래그 + 폴더에 같은 이름이 이미 있으면 **건너뛴다**(중단된 실행의 재시도가 " 2" 사본을 만들지 않도록)

**차이 하나:** 회전에 넣는 건 「직전 버전」이 아니라 **방금 저장한 바이트**다.
되읽기가 모든 저장의 IO를 두 배로 만들고, 세는 방식도 달라지기 때문 — 근거는
ADR-012. 그래서 목록의 첫 항목은 현재 파일과 같고, 「이 버전 직전」은 두 번째다.

**차이 둘:** 라이브러리 UI 제거는 **데스크톱에서만**이다. 웹 빌드에는 파일이 없어
라이브러리가 프로젝트를 여러 개 두는 유일한 수단이라, 마이그레이션 자체가 정의되지
않는다 (전제 1 + 「새 기능은 `isTauri()` 뒤에」). 근거는 ADR-013.
...
| 층 | 어디 | 무엇 |
|---|---|---|
| Rust 유닛 (31개) | `cargo test` | 임의 경로 원자적 저장 + 실패 시 원본 보존, `.bak` 1회, 백업 회전/가지치기, 종료 가드 상태 기계(생존 타임아웃·재진입·세대), 인스턴스 잠금(살아 있는 피어 vs 남은 파일), 경로/이름 검증 |
| TS 순수 유닛 | `src/lib/documentModel.test.ts` | dirty 해시(`updatedAt` 제외, undo 복귀), 이름↔경로, 충돌 회피, Recents 추가/중복제거/유실/파싱 방어, 백업 스탬프 |
| TS IO 유닛 | `src/lib/documentIO.test.ts` | 저장 경로·clean일 때 무동작·실패 시 dirty 유지, `.bak` 순서, 3버튼 게이트 4갈래, 종료 가드 웹뷰 절반(ack 먼저, 저장 실패 시 종료 안 함) |
| 라운드트립 | `src/lib/projectBundle.test.ts` | 저장 → 재오픈 시 프로젝트 동일(같은 해시) + 참조 이미지 전부 존재 + 담지 못한 이미지 보고 + 파일이 쓰인 스키마 보고 |
| 데스크톱 e2e (27개) | `npm run test:doc` | 살아 있는 앱을 브리지로 몰아서: 새 프로젝트가 곧 파일 · 패치가 dirty · 저장이 clean · 미저장 전환 거부 · A↔B 전환 · 미러의 `docPath` · Recents 파일과 썸네일 캐시 · 백업 회전 · **v4 파일의 `.bak` 바이트 비교** · **읽기 전용 폴더 저장 실패 후 원본 무손상** · **스크린샷이 `<config>/autosave-images/`에 실제로 들어가는지**(Phase 0 잔여였던 IPC 이음새) |

> 이 하네스를 돌리다 **저장 경로의 실제 버그를 하나 잡았다.** `blobToBase64`가
> `FileReader`를 쓰고 있었는데, WebKit은 백그라운드로 판단한 창을 스로틀하고 그
> 상태의 `FileReader`는 "The I/O read operation failed"로 그냥 실패한다 — 앱이
> 다른 창 뒤에 있을 때 에이전트의 저장이 통째로 안 되던 원인. `arrayBuffer()`로
> 바꿨다. 파일로 나가는 모든 바이트(프로젝트 저장 · 미러 이미지 · PNG 내보내기)가
> 이 함수를 지난다.

### 사람이 확인할 것 (자동화 불가)

- [ ] `⇧⌘S` 저장 패널이 `.studio.zip`을 어떻게 보여주는지, 그리고 확장자를 빼고
      입력했을 때 앱이 띄우는 덮어쓰기 확인이 실제로 뜨는지
- [ ] dirty 상태에서 **⌘Q**(그리고 창의 빨간 버튼) → 3버튼이 뜨고 `취소`가 종료를
      막는지. AppleEvent quit은 이 경로를 타지 않으므로 스크립트로는 확인 불가다
- [ ] File 메뉴에 ⌘N/⌘O/⌘S/⇧⌘S/⌘W가 보이고, 언어를 토글하면 라벨이 바뀌는지
- [ ] 창 제목이 파일 이름이고 dirty면 앞에 `•`가 붙는지
- [ ] 두 번째 인스턴스를 띄웠을 때 경고창 문구(현재 `osascript` 경고라 "osascript"
      이름으로 뜬다 — ADR-011)

~~Phase 0 잔여(스크린샷 → `<config>/autosave-images/`)~~ 는 이제 `test:doc`이 덮는다.
