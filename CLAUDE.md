...
Layer order (bottom → top): Background → Shapes(back) → Ornaments → Screenshot → DeviceFrame → Headline/Subheadline → Shapes(front) → ExternalImage → HighlightSource (dashed) → HighlightPopup → Badge. Generic shapes (`Slide.shapes`: rect/ellipse/line/arrow) pick their band per shape via `layer: 'back' | 'front'` — one `layerName` (`shape`), band = insertion order. Highlights render *after* text + device frame so the magnified card floats above the bezel while the badge stays top-most. Connector lines between source and popup are part of the data model (`Highlight.popup.showConnectorLine`) but not yet rendered.
