...
export function buildImageNamingGuide(
  ...
  const slide1 = [source, ...targets]
    .map((l) => (l.code === source.code ? `1.${l.code}.png${t('(베이스)')}` : `1.${l.code}.png`))
    .join(', ')
  return [
    t('이미지 파일명 규칙 (벌크 업로드)'),
    t('• 형식: {슬라이드번호}[-설명].{언어}.png — 모든 파일에 언어 접미사 필요'),
    t('• 기준 언어({label}, {code})로 들어온 파일 = 슬라이드 베이스, 나머지 = 언어별 추가본', { label: source.label, code: source.code }),
    t('• 슬라이드 1 예시: {examples}', { examples: slide1 }),
    t('• 설명 접미사 허용: 01-home.{src}.png, 02-add-pdf.{tgt}.png', { src: source.code, tgt: targets[0]?.code ?? source.code }),
  ].join('\n')
}
...
export function parseImageName(
  ...
  if (!Number.isInteger(slide) || slide < 1) {
    return { error: t('슬라이드 번호를 읽을 수 없음: "{filename}"', { filename }) }
  }
  if (dot < 0) {
    return { error: t('언어 접미사가 필요함 (예: 01.en.png): "{filename}"', { filename }) }
  }
  const locale = name.slice(dot + 1)
  if (!knownLocales.has(locale)) {
    return { error: t('지원하지 않는 언어 "{locale}": "{filename}"', { locale, filename }) }
  }
  return { slide, locale }
}
