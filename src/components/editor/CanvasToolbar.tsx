interface Props {
  canUndo: boolean
  canRedo: boolean
  /** Locale mode: snapshots don't route, so the buttons stay off like the keys. */
...
export function CanvasToolbar({ canUndo, canRedo, actionsDisabled, onUndo, onRedo }: Props) {
  return (
    <div className="flex items-center gap-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-2 py-1">
      <button type="button" title="Undo (Cmd+Z)" onClick={onUndo} disabled={!canUndo || actionsDisabled} className={btn}>
        Undo
      </button>
