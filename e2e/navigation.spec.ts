...
import { clearAppState, createProject, openMoreMenu } from './helpers'
...
test('초기화 버튼은 프로젝트 없으면 표시되지 않음', async ({ page }) => {
  // No project → the toolbar's More menu (which holds 초기화) isn't rendered.
...
test('프로젝트 생성 후 초기화 버튼 표시됨', async ({ page }) => {
  ...
  await expect(page.getByRole('menuitem', { name: '초기화' })).toBeVisible()
})
