...
// Bulk image import on the Localize page: a multi-file pick routes each file by
// its name — "1.png" -> slide 1 base screenshot, "1.ja.png" -> slide 1 ja
// override (base sorted first so the override can attach in the same batch).
...
test('이미지 일괄 가져오기: 기기 불일치는 덮어쓰지 않고 건너뜀', async ({ page }) => {
  ...
  // First import sets slide 1 to an iPhone screenshot.
  await page.locator('input[accept="image/*"][multiple]').setInputFiles([
    { name: '1.png', mimeType: 'image/png', buffer: iphone },
  ])
  await expect(page.getByText(/이미지를 가져왔습니다/)).toBeVisible()

  // An iPad-aspect image for the same slide must be skipped, not silently swapped.
  await page.locator('input[accept="image/*"][multiple]').setInputFiles([
    { name: '1.png', mimeType: 'image/png', buffer: ipad },
  ])
