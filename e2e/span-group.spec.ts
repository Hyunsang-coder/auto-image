...
test('링크하면 기기가 seam(캔버스 중앙)에 정렬됨 — 비중앙 템플릿(hero-bleed)에서도', async ({ page }) => {
  // The leader (slide 1) defaults to the hero-bleed template, whose single-slide
  // device sits right-of-center (centerX = cw * 0.7). Inside a 2-page span the
  // device must straddle the seam (cw / 2) so the screenshot spans both pages
  // evenly instead of being pushed onto page 2.
  await page.getByTitle('옆 슬라이드와 한 장으로 묶기').first().click()
  await page.waitForFunction(() => {
    const c = document.querySelector('canvas.upper-canvas') as HTMLCanvasElement | null
    return c?.width === 880
