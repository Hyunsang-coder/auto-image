...
test('로컬라이즈에서 span 양쪽이 각자 슬라이드 번호로 행을 가짐', async ({ page }) => {
  await page.getByTitle('옆 슬라이드와 한 장으로 묶기').first().click()
  await expect(page.getByTitle('🔗 2-page span')).toBeVisible()
  ...
  // Texts are per-slide: the leader's caption rows sit under "1" and the
  // follower's under "2" (no combined "1·2" label anymore).
  await expect(page.locator('td', { hasText: /^1$/ })).toBeVisible()
  await expect(page.locator('td', { hasText: /^2$/ })).toBeVisible()
  await expect(page.locator('td', { hasText: /^1·2$/ })).toHaveCount(0)
  // Slide #3 (the un-grouped one) keeps its plain "3" label.
  await expect(page.locator('td', { hasText: /^3$/ })).toBeVisible()
})

test('Follower 텍스트는 패널에서 편집되고 오른쪽 페이지에 렌더링됨', async ({ page }) => {
  await page.getByTitle('옆 슬라이드와 한 장으로 묶기').first().click()
  await page.waitForFunction(() => {
    const c = document.querySelector('canvas.upper-canvas') as HTMLCanvasElement | null
    return c?.width === 880
  ...
  await page.waitForFunction(() => {
    ...
    const t = editor.canvas.getObjects().find(
      (o) =>
        (o as unknown as { layerName?: string }).layerName === 'text' &&
        (o as unknown as { owner?: string }).owner === 'follower',
    ) as (import('fabric').Textbox) | undefined
    return !!t && t.text === '오른쪽 텍스트' && t.getCenterPoint().x > editor.canvas.width! / 2
  })

  // The leader's caption is untouched and stays on the left page.
  const leader = await page.evaluate(() => {
    const editor = (window as unknown as { __editor: { canvas: import('fabric').Canvas } }).__editor
    const t = editor.canvas.getObjects().find(
      (o) =>
        (o as unknown as { layerName?: string }).layerName === 'text' &&
        (o as unknown as { owner?: string }).owner === 'leader',
    ) as import('fabric').Textbox
    return { text: t.text, centerX: t.getCenterPoint().x, seam: editor.canvas.width! / 2 }
  })
  expect(leader.text).toBe('당신의 헤드라인')
  expect(leader.centerX).toBeLessThan(leader.seam)
})
