...
test('크기 입력은 빈칸 상태를 0으로 저장하지 않고 완성된 숫자만 반영', async ({ page }) => {
  ...
  await sizeInput.evaluate((el: HTMLInputElement) => el.select())
  await page.keyboard.press('Backspace')

  await expect(sizeInput).toHaveValue('')
  const whileEmpty = await page.evaluate(
    () => JSON.parse(localStorage.getItem('auto-image:project')!).state.project.slides[0].texts[0].style.fontSize,
  )
  expect(whileEmpty).toBe(40)
  await expect.poll(async () => (await readHeadline(page))?.fontSize ?? 0).toBe(40)

  await page.keyboard.type('96')

  await expect(sizeInput).toHaveValue('96')
  await expect
    .poll(async () =>
      JSON.parse(await page.evaluate(() => localStorage.getItem('auto-image:project')!)).state.project.slides[0].texts[0].style.fontSize,
    )
    .toBe(96)
  await expect.poll(async () => (await readHeadline(page))?.fontSize ?? 0).toBe(96)
})
...
test('fit-to-box: 모서리 스케일은 줄어든 표시 크기로 저장 크기를 낮추지 않음', async ({ page }) => {
  ...
  expect(before.fontSize!).toBeLessThan(300)

  await selectLayer(page, 'text')
  const br = await controlPos(page, 'text', 'br')
  await drag(page, br, { x: br.x + 80, y: br.y + 80 })
...
test('fit-to-box: 가장 긴 단어보다 좁게 줄이면 글자가 더 작아짐 (드래그 데드락 없음)', async ({ page }) => {
  await page.locator('label:has-text("박스 너비에 맞춤") input[type=checkbox]').check()
  await page.locator('textarea').first().fill('Comprehensive weather information')
  await expect.poll(async () => (await readHeadline(page))?.text).toBe('Comprehensive weather information')

  // The widest word at the design size (~287px) used to deadlock the resize:
  // Fabric clamps the Textbox at the widest word for the CURRENT font, so the
  // width never dropped and the fit never engaged. The resize handler now
  // fits against the pointer-requested width, PPT-style.
  await selectLayer(page, 'text')
  const ml = await controlPos(page, 'text', 'ml')
  await drag(page, ml, { x: ml.x + 260, y: ml.y })
  await expect
    .poll(async () => (await readHeadline(page))?.fontSize ?? 99, { timeout: 5_000 })
    .toBeLessThan(30)
  expect((await readHeadline(page))!.width!).toBeLessThan(200)
})
